...
export const IncidentNotificationModal = ({
  incident,
  onAccept,
  onRefuse,
  onClose,
  ...
  const centerPosition = [
    zone?.latitude_centre || 30.42,
    zone?.longitude_centre || -8.85,
  ];
  ...
  useEffect(() => {
    ...
    const diffSeconds = Math.max(
      0,
      Math.floor((triggerTime + 5 * 60 * 1000 - now) / 1000),
    );
  ...
  const handleTimeoutRefuse = async () => {
    ...
    try {
      await axios.patch(
        `/api/alerts/${id_alerte}/refuse`,
        { id_equipe: myTeam.id_equipe, raison: "Timeout (Sans réponse)" },
        {
  ...
  const handleAccept = async () => {
    ...
    try {
      await axios.patch(
        `/api/alerts/${id_alerte}/assign`,
        { id_equipe: myTeam.id_equipe },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        },
      );
  ...
  const handleRefuse = async (reason) => {
    ...
    try {
      await axios.patch(
        `/api/alerts/${id_alerte}/refuse`,
        { id_equipe: myTeam.id_equipe, raison: reason },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        },
      );
        ...
        >
          ...
          <div className="bg-rose-600 text-white px-6 py-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-black tracking-widest text-lg uppercase">
                🔥 Incident Détecté
              </h2>
            </div>
            <div className="flex items-center gap-4">
              <div
                className={`font-mono font-bold text-sm px-3 py-1 rounded-full ${timeLeft < 60 ? "bg-red-800 animate-pulse" : "bg-rose-700"}`}
              >
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-6">
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-500 font-mono bg-slate-50 px-3 py-1 rounded-lg w-fit">
                    ...
                    {centerPosition[0].toFixed(4)},{" "}
                    {centerPosition[1].toFixed(4)}
                  </div>
                  <p className="text-xs text-slate-400 mt-2 font-medium">
                    📍 à ~{distanceToCaserne}km de la caserne
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col justify-center items-center text-center relative">
                ...
                <p className="text-slate-400 text-[10px] uppercase tracking-widest font-bold mb-2 pt-2">
                  Température
                </p>
                <h4 className="text-6xl font-black text-rose-600 mb-2">
                  {temperature_detectee}°
                  <span className="text-3xl text-rose-400">C</span>
                </h4>
                <div className="bg-rose-50 text-rose-700 text-xs px-3 py-1.5 rounded-lg border border-rose-100 font-medium">
                  Seuil: {capteur.seuil_alerte_celsius}°C — Capteur:{" "}
                  {capteur.numero_serie}
                </div>
              </div>

              {/* Propagation */}
              <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm relative">
                <h4 className="text-slate-400 text-[10px] uppercase tracking-widest font-bold mb-4">
                  Propagation Estimée
                </h4>
                <div className="flex items-center gap-4">
                  <div className="relative w-16 h-16 flex-shrink-0">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle
                        ...
                        cy="32"
                        r="28"
                        className="stroke-slate-100 stroke-[6px] fill-none"
                      />
                      <circle
                        cx="32"
                        cy="32"
                        r="28"
                        className="stroke-rose-500 stroke-[6px] fill-none"
                        strokeDasharray="175"
                        strokeDashoffset={
                          175 - (175 * probabilite_propagation_pct) / 100
                        }
                        strokeLinecap="round"
                      />
                    </svg>
                ...
                {niveau_urgence && (
                  <div
                    className={`mt-4 w-full text-center px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-widest 
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-1.5 text-slate-500">
                    ...
                    <span className="text-xs font-semibold">Humidité</span>
                  </div>
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-1.5 text-slate-500">
                    ...
                    <span className="text-xs font-semibold">Vent (km/h)</span>
                  </div>
                  <span className="text-lg font-black text-slate-800">
                    {meteo.vitesse_vent_kmh}
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-1.5 text-slate-500">
                    <Compass className="w-4 h-4 text-emerald-600" />{" "}
                    <span className="text-xs font-semibold">Dir. Vent</span>
                  </div>
                  <span className="text-lg font-black text-slate-800">
                    {meteo.direction_vent_deg}°
                  </span>
                </div>
                <div className="flex flex-col gap-1 relative">
                  <div className="flex items-center gap-1.5 text-slate-500">
                    <Flame className="w-4 h-4 text-amber-500" />{" "}
                    <span className="text-xs font-semibold">Sécheresse</span>
                  </div>
                  <span
                    className={`text-lg font-black ${meteo.indice_secheresse > 3.5 ? "text-rose-600" : "text-slate-800"}`}
                  >
                    {meteo.indice_secheresse}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-4 flex items-center justify-between border border-slate-100">
              <div>
                <h4 className="text-slate-900 font-bold mb-1">
                  Destinataire : {myTeam.nom_equipe || "Équipe Connectée"}
                </h4>
                <p className="text-xs font-medium text-slate-500 mb-2">
                  Comptage effectif : {myTeam.nombre_dispo || 4} pompiers
                  disponibles
                </p>
          ...
          <div className="bg-white border-t border-slate-100 p-6 shrink-0 relative">
            {showRefuseReasons ? (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {[
                    ...
                    <button
                      ...
                      className="px-3 py-2 text-xs font-bold bg-slate-50 border border-slate-200 text-slate-600 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 rounded-xl transition-colors"
                    >
                      {r}
                    </button>
                  ))}
                ...
                <button
                  onClick={() => setShowRefuseReasons(false)}
                  className="w-full text-center text-xs text-slate-400 font-semibold hover:text-slate-600 underline"
                >
                  Annuler
                </button>
              </motion.div>
