...
const navLinks = [
  ...
  {
    path: "/pompier/alertes",
    name: "Alerts",
    icon: Bell,
    badgeType: "alertes",
  },
  { path: "/pompier/map", name: "Map", icon: MapIcon },
  { path: "/pompier/incidents", name: "Incidents", icon: Flame },
  {
    path: "/pompier/notifications",
    name: "Notifications",
    icon: Bell,
...
export default function PompierLayout() {
  ...
  const [user, setUser] = useState({
    ...
    grade: storedUser.role || "POMPIER",
    equipe: { nom_equipe: "Response" },
  });
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () => {
      console.log("Socket connected (Pompier)");
    });
            ...
            <div className="mb-6 flex items-center gap-3 bg-slate-50 p-3 rounded-2xl">
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <p className="text-xs text-slate-500 uppercase tracking-wider">
                  {user.grade} - Eq. {user.equipe.nom_equipe}
                </p>
              </div>
            </div>
          </div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  {link.badgeType === "alertes" && activeAlerts > 0 && (
            ...
            .map((link) => {
              ...
              return (
                <NavLink
                  ...
                  to={link.path}
                  className={({ isActive }) =>
                    `flex flex-col items-center p-2 rounded-xl ${isActive ? "text-emerald-600 bg-emerald-50" : "text-slate-400"}`
                  }
                >
                  <Icon size={24} />
