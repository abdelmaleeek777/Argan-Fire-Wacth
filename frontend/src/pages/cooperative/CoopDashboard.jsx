...
export default function CoopDashboard() {
    ...
    try {
      ...
      setStats({
        ...
        totalAlerts: alerts.filter(
          (a) => a.statut === "active" || a.statut === "OUVERTE",
        ).length,
  ...
  const fetchChartData = async () => {
    try {
      ...
      const [tempRes, alertsRes, sensorsRes] = await Promise.all([
        axios
          .get(
            `${API_BASE}/cooperative/${coopId}/charts/temperature?duration=${duration}`,
            { headers },
          )
          .catch(() => ({ data: [] })),
        axios
          .get(
            `${API_BASE}/cooperative/${coopId}/charts/alerts?duration=${duration}`,
            { headers },
          )
          .catch(() => ({ data: [] })),
        axios
          .get(
            `${API_BASE}/cooperative/${coopId}/charts/sensors?duration=${duration}`,
            { headers },
          )
          .catch(() => ({ data: [] })),
      ]);
      ...
      setTempData(
        tempRes.data?.length > 0 ? tempRes.data : generateStaticData("temp"),
      );
      setAlertData(
        alertsRes.data?.length > 0
          ? alertsRes.data
          : generateStaticData("alerts"),
      );
      setSensorData(
        sensorsRes.data?.length > 0
          ? sensorsRes.data
          : generateStaticData("sensors"),
      );
    } catch (err) {
  ...
  const generateStaticData = (type) => {
    ...
    return days.map((day) => {
      ...
      if (type === "sensors")
        return { name: day, readings: 50 + Math.floor(Math.random() * 100) };
      return { name: day };
    });
  };
    ...
    <div className="space-y-8 pb-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-[#6B7468] font-bold mt-1">
            Real-time statistics for{" "}
            {user.cooperative_name || "your cooperative"}.
          </p>
        </div>
        <div className="flex gap-3">
          <button
            ...
            className="p-3 bg-[#F8F7F2] border border-[#4F5C4A]/[0.10] text-[#6B7468] rounded-[16px] hover:bg-[#DCE3D6] hover:text-[#1F2A22] transition-colors shadow-sm"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {statCards.map((card, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-[#F8F7F2] p-6 rounded-[24px] border border-[#4F5C4A]/[0.10] shadow-[0_4px_20px_rgba(31,42,33,0.03)] hover:shadow-[0_8px_24px_rgba(31,42,33,0.06)] hover:-translate-y-1 transition-all group overflow-hidden relative"
            >
              {/* Decorative blob */}
              <div
                className="absolute right-[-10%] top-[-10%] w-24 h-24 rounded-full blur-[24px] opacity-20 pointer-events-none"
              ...
              <div className="relative z-10">
                <p
                  className="text-[11px] font-[800] uppercase tracking-widest mb-1"
      ...
      <div className="flex items-center justify-end gap-3 px-2">
        <span className="text-[11px] font-[800] text-[#6B7468] uppercase tracking-widest">
          Timeframe
        </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#F8F7F2] border border-[#4F5C4A]/[0.10] rounded-[28px] p-7 shadow-[0_4px_20px_rgba(31,42,33,0.03)]">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-[16px] font-[800] text-[#1F2A22]">
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis
                  ...
                  axisLine={false}
                  tickLine={false}
                  tick={{ fontSize: 11, fill: "#6B7468", fontWeight: 600 }}
                />
                <YAxis
                  yAxisId="left"
                  axisLine={false}
                ...
                />
                <Tooltip
                  cursor={{ fill: "rgba(78, 107, 74, 0.05)" }}
                  contentStyle={{
                    borderRadius: "16px",
                    border: "1px solid rgba(79,92,74,0.1)",
                    boxShadow: "0 8px 24px rgba(31,42,33,0.08)",
                ...
                <Line
                  ...
                  stroke="#B55A3C"
                  strokeWidth={2}
                  dot={{ r: 3, fill: "#B55A3C", strokeWidth: 0 }}
                  activeDot={{ r: 5 }}
                />
                <Line
                  yAxisId="right"
                  ...
                  stroke="#4E6B4A"
                  strokeWidth={2}
                  dot={{ r: 3, fill: "#4E6B4A", strokeWidth: 0 }}
                  activeDot={{ r: 5 }}
                />
              </ComposedChart>
            </ResponsiveContainer>
        ...
        <div className="lg:col-span-2 bg-[#F8F7F2] border border-[#4F5C4A]/[0.10] rounded-[28px] p-7 shadow-[0_4px_20px_rgba(31,42,33,0.03)]">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-[16px] font-[800] text-[#1F2A22]">
              Global Telemetry Transmissions
            </h3>
          </div>
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Tooltip
                  cursor={{ stroke: "rgba(78, 107, 74, 0.2)" }}
                  contentStyle={{
                    borderRadius: "16px",
                    border: "1px solid rgba(79,92,74,0.1)",
                    boxShadow: "0 8px 24px rgba(31,42,33,0.08)",
