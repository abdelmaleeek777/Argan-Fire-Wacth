          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                {modal.type === "view" && <span className="flex items-center gap-2"><Eye className="w-5 h-5 text-blue-500"/> Informations du Compte</span>}
                {modal.type === "block" && <span className="flex items-center gap-2"><Lock className="w-5 h-5 text-amber-500"/> Bloquer le compte</span>}
                {modal.type === "unblock" && <span className="flex items-center gap-2"><Unlock className="w-5 h-5 text-emerald-500"/> Débloquer le compte</span>}
                {modal.type === "delete" && <span className="flex items-center gap-2 text-rose-600"><AlertTriangle className="w-5 h-5"/> Confirmer la suppression</span>}
              </h3>
            ...
            <div className="p-6">
              {modal.type === "view" ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 flex items-center gap-1"><Mail className="w-4 h-4"/> {modal.user.email}</p>
                    </div>
                  </div>
                ...
                <div className="text-center">
                  ...
                  <p className="text-lg font-bold text-slate-900 mb-4">{modal.user.name} ({modal.user.email})</p>
                  ...
                  {modal.type === "delete" && (
                    <div className="bg-rose-50 text-rose-600 p-4 rounded-xl text-sm font-medium border border-rose-100 mt-6 text-left flex gap-3">
                      ...
                      Attention: Cette action est irréversible. Toutes les données associées à cet utilisateur seront supprimées.
                    </div>
                  )}
                  {modal.type === "block" && (
                    <div className="bg-amber-50 text-amber-700 p-4 rounded-xl text-sm font-medium border border-amber-100 mt-6 text-left flex gap-3">
                      ...
                      L'utilisateur ne pourra plus se connecter et ne recevra plus d'alertes jusqu'à son déblocage.
                    </div>
                  )}
                </div>
              )}
